...
select year(order_date), count(distinct customer_key) as num_customers, sum(sales_amount ) as tot_sales, sum(quantity) as tot_quantity
from gold_sales
where order_date is not null
AND order_date <> '' #to avoid empty cells
group by year(order_date)
...
select order_year, month(order_month) as order_month, tot_sales, sum(tot_sales) over(partition by order_year order by order_month) as running_tot_sales
from
(select 
